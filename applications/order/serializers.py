...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        # {'items': [OrderedDict([('product', < Product: 1111 >), ('quantity', 5)]),
        #            OrderedDict([('product', < Product: Xiami mi 10 note >), ('quantity', 1)])],
        #            'total_cost': 0,
        #  'delivery_address': 'Bishkek', 'contact_number': '996772662864', 'first_name': 'Tom'}
    ...
    def to_representation(self, instance):
        ...
        rep['products'] = OrderProductSerializer(OrderProduct.objects.filter(order=instance.id),
                                                 many=True).data
        return rep